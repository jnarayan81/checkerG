...
print  O1 ("Name\tRepeat Start\tRepeat End\tPeriod Size\tCopy No.\tAlignment Score\tConsensus\n");
...
my ($rep_start, $rep_end, $period_size, $copy_no, $pattern_size, $percent_match, $percent_indel, $align_score, $a_percent, $c_percent, $g_percent, $t_percent, $entropy, $consensus, $repeat);			
			chomp($line);
if  ($line =~ /^Sequence/) { my @seqN = split('\:', $line); $seqN[1] =~ s/^\s+|\s+$//g; $seqName=$seqN[1];}
			if ($line =~ /^[0-9]/) {
			my @arr = split(" ", $line);
			...
			$consensus = $arr[13];
			$repeat = $arr[14];
			print O1 ("$seqName\t$rep_start\t$rep_end\t$period_size\t$copy_no\t$align_score\t$consensus\n");
			}
